function mCADload(boat)
% Load ArDrone 3D CAD model

boat.pCAD.obj = loadawobj('catamaran.obj');
boat.pCAD.mtl = loadawmtl('catamaran.mtl');

% Flags
boat.pCAD.flagLoaded  = 1;  % 3D image loaded
boat.pCAD.flagCreated = 0;  % 3D image created using Matlab Patch function
boat.pCAD.flagLines   = 1;  % Illustrate lines between ArDrone and surface


vExt = minmax(boat.pCAD.obj.v);
mExt = mean(vExt,2);


for ii = 1:3
    boat.pCAD.obj.v(ii,:) = (boat.pCAD.obj.v(ii,:) - mExt(ii))/100;
end

boat.pCAD.obj.v = [1 0 0; 0 0 -1; 0 1 0]*boat.pCAD.obj.v;   
boat.pCAD.obj.v = [0 1 0; -1 0 0; 0 0 1]*boat.pCAD.obj.v;
boat.pCAD.obj.v(3,:) = boat.pCAD.obj.v(3,:) - (3 + min(boat.pCAD.obj.v(3,:)))*ones(1,length(boat.pCAD.obj.v(3,:)));
