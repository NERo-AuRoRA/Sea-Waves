...
while toc(total)<tmax
    % Controle de posicao
%     if toc(total) > 3*tmax/4
        boat.pPos.Xd(1) = 120;
        boat.pPos.Xd(2) = 100;
        boat.pPos.Xd(3) = boat.pPos.X(3);
% %         boat.pPos.Xd(6) = 0;
%     elseif toc(total) > 2*tmax/4
%         boat.pPos.Xd(1) = 120;
%         boat.pPos.Xd(2) = 120;
%         boat.pPos.Xd(3) = boat.pPos.X(3);
% %         boat.pPos.Xd(6) = 0;
%     elseif toc(total) > tmax/4
%         boat.pPos.Xd(1) = 100;
%         boat.pPos.Xd(2) = 120;
%         boat.pPos.Xd(3) = boat.pPos.X(3);
% %         boat.pPos.Xd(6) = 0;
...
%     w = 0.01;
    % Trajetória desejada
%     boat.pPos.Xd(1) = 40*sin(2*pi*w*tt)+100;            % x
%     boat.pPos.Xd(7) = 40*2*pi*w*cos(2*pi*w*tt);     % dx
%     boat.pPos.Xd(2) = 40*sin(2*pi*2*w*tt)+100;          % y
%     boat.pPos.Xd(8) = 40*2*pi*2*w*cos(2*pi*2*w*tt); % dy
%     boat.pPos.Xd(3) = boat.pPos.X(3);              % z
%     boat.pPos.Xd(6) = -atan2(boat.pPos.Xd(8),boat.pPos.Xd(7));
%     boat.pPos.Xd(5) = 0;
%     boat.pPos.Xd(4) = boat.pPos.X(4)*0.05;

        
    %% Atualiza posição/estado da onda e barco:
                          % n,i,j,dt,c,k,H,oldH,fix,cont,connect
                          k = 12*exp(-toc(total)/tmax);
    [aceleracoes,newH,Ekin,Epot]=Wave(n,i,j,0.05,k,0.2,H,oldH,0,0,0,Ekin,Epot);
...
%     roll           | [-1,1] <==> [-15,15] degrees
    ...
    if toc(total)>3 % esperar "transitorio"
        ...
        cgains = [0.5 2 0.5 2 5 2; 1*5 15*5 1*5 15*5 1*5 2.5*5]*3000;  
        cUnderActuatedController(boat,cgains);
        ...
        if toc(total)>10 && rem(toc(total),5)<1.5
            
%             forca = forca*10*exp(10*toc(total)/tmax)
            [H,globOH] = add_Catamara(boat.pPos.X(1),boat.pPos.X(2),n,H,boat.pPos.X(3)+boat.pPesoVirtual);
        end
        ...
        if toc(t)>tempo
           ...
           for ponto = 1:size(indices_PdC,2)
               [newH,globOH] = add_Catamara(indices_PdC(1,ponto),indices_PdC(2,ponto),n,newH,-boat.pPos.X(3)+forca*(1/size(indices_PdC,2)));
           end
...
%     pontosAgua = plot3(indices_PdC(1,:),indices_PdC(2,:),newH(indices_PdC(1,:),indices_PdC(2,:)),'b*','LineWidth', 12,'MarkerSize', 10);
    traj= plot3(boat.pPos.Xd(1),boat.pPos.Xd(2),boat.pPos.Xd(3),'rx','LineWidth', 5,'MarkerSize', 10);
    %Mostra barco e onda:
    boat.mCADplot();
